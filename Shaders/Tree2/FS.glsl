...
void main() {
    ...
    //ambient
    vec3 ambient = light.ambient * v_colour;

    //diffuse
    float diff = max(dot(norm, halfwayDir), 0.0);
    vec3 diffuse = light.diffuse * diff * v_colour;
    ...
    vec3 specular = light.specular * spec * v_colour;
    ...
    vec3 res = (shadow * (diffuse + specular) + ambient) * v_colour;
